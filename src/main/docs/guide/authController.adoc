Create `AuthController.groovy`. When the user logs in successfully with Google, `AuthController.index` is invoked.

NOTE: The path to `AuthController.index` is used in `frontendCallbackUrl` in `application.groovy`.
...
The GSP of both actions perform simple redirect to the home page. We do the redirect in the
client side to ensure cookies are correctly set or deleted.
...
[source,groovy]
.grails-app/views/auth/logout.gsp
----
include::{sourceDir}/grails-app/views/auth/logout.gsp[indent=0]
----